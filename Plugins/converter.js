    ...
    switch (inputCMD) {
      ...
      case "toimage":
        if (!m.quoted && !/webp/.test(mime)) {
          await doReact("❔");
          return m.reply(
            `Please reply to a *Non-animated* sticker to convert it to an image`
          );
        }
        await doReact("🎴");
        let mediaMess = await Arashi.downloadAndSaveMediaMessage(quoted);
        ...
        exec(`ffmpeg -i ${mediaMess} ${ran}`, (err) => {
          ...
          if (err) {
            Arashi.sendMessage(
              m.from,
              {
                text: `Please mention a *Non-animated* sticker to process!\n\nOr use *${prefix}togif* / *${prefix}tomp4* to process *Animated* stickers!`,
              },
              { quoted: m }
            );
            return;
      ...
      case "tomp4":
        if (!m.quoted && !/webp/.test(mime)) {
          await doReact("❔");
          return reply(
            `Please reply to an *Animated* sticker to convert it to a video!`
          );
        }
        await doReact("🎴");
        let mediaMess2 = await Arashi.downloadAndSaveMediaMessage(quoted);
      ...
      case "togif":
        if (!m.quoted && !/webp/.test(mime)) {
          await doReact("❔");
          return m.reply(
            `Please reply to an *Animated* sticker to convert it to a gif!`
          );
        }
        await doReact("🎴");
        let mediaMess3 = await Arashi.downloadAndSaveMediaMessage(quoted);
      ...
      case "tomp3":
        if (/document/.test(mime)) {
          await doReact("❌");
          return m.reply(
            `Send/Reply to the video/audio you want to convert into MP3 with the caption *${prefix}tomp3*`
          );
        }
      ...
      case "tourl":
        if (!m.quoted) {
          await doReact("❔");
          return m.reply(
            `Please provide an *image* / *video* to generate a link with the caption *${prefix}tourl*`
          );
        }
        let media5 = await Arashi.downloadAndSaveMediaMessage(quoted);
        if (/image/.test(mime)) {
          await doReact("🔗");
      ...
      case "imgtopdf":
        if (/image/.test(mime)) {
          await doReact("📑");
          let mediaMess4 = await Arashi.downloadAndSaveMediaMessage(quoted);
