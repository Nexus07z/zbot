    ...
    switch (inputCMD) {
      ...
      case "sticker":
        if (/image/.test(mime)) {
          ...
          Arashi.sendMessage(m.from, { sticker: stickerBuffer }, { quoted: m });
      ...
      case "stickermeme":
        if (/image/.test(mime)) {
          ...
          await doReact("📮");
          media = await Arashi.downloadAndSaveMediaMessage(quoted);
      ...
      case "quote":
        ...
        if (m.quoted) {
          try {
            userPfp = await Arashi.profilePictureUrl(m.quoted.sender, "image");
          } catch (e) {
            userPfp = botImage3;
          }
        } else {
          try {
            userPfp = await Arashi.profilePictureUrl(m.sender, "image");
          } catch (e) {
            userPfp = botImage3;
          }
        }
