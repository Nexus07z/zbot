...
module.exports = {
  ...
  start: async (Arashi, m, { inputCMD, text, doReact, prefix, pushName }) => {
    switch (inputCMD) {
      case "google":
      case "search":
        if (!text) {
      ...
      case "lyrics":
        if (!text) {
          await doReact("❔");
          return m.reply(
            `Please provide an lyrics Search Term !\n\nExample: *${prefix}lyrics Be Alright*`
          );
        }
        await doReact("📃");
        let result = await axios.get(
          "https://hridoy0016-scrappy-api.vercel.app/lyrics?search=" + text
        );
        let lyrics = result.data.lyrics;
        let thumbnail = result.data.thumbnail;

        let resText2 = `  *『  ⚡️ Lyrics Search Engine ⚡️  』*\n\n\n_Search Term:_ *${text}*\n\n\n*📍 Lyrics:* \n\n${lyrics}\n\n\n_*Powered by:*_ *Scrappy API - by Hridoy*\n\n_*Url:*_ https://github.com/Hridoy0016/Scrappy-API \n`;
        await Arashi.sendMessage(
          m.from,
          {
            image: {
    ...
    var txt2 = `*🏮 YouTube Search Engine 🏮*\n\n_🧩 Search Term:_ *${text}*\n\n*📌 Total Results:* *${search.all.length}*\n`;*/
      ...
      case "getsticker":
        ...
        Arashi.sendMessage(m.from, { sticker: stickerBuffer2 }, { quoted: m });
        break;
