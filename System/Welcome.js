...
module.exports = async (Arashi, anu) => {
  try {
    let metadata = await Arashi.groupMetadata(anu.id);
    ...
    for (let num of participants) {
      try {
        ppuser = await Arashi.profilePictureUrl(num, "image");
      } catch {
        ppuser = botImage4;
      }
...
*Thank You.*
  `;
        if (WELstatus) {
          await Arashi.sendMessage(anu.id, {
            image: { url: ppuser },
            caption: Arashitext,
            mentions: [num],
          });
